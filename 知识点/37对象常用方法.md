...
## 目录
1. Object.keys()  遍历属性名返回数组 用来找到对象自身可枚举的属性名
2. Object.assign() 将源对象可枚举的属性复制到目标对象
3. Object.create() 创建对象
4. Object.defineProperty() 在一个对象上新增属性或者修改属性
5. Object.entries() 遍历对象返回键值对组成的数组 把对象转化为一个key-value数组 二维数组
...
21. values用来找到对象自身可枚举的属性值

## 1. Object.keys()
```javascript
/*
用for循环也可以循环出来，但是用for循环就合理吗？
forin循环会把原型上的属性也遍历出来，我们其实只想遍历他自身上的属性
1. 这个属性是可枚举的
2. 必须是自身（不带原型）具有的属性
...
```javascript
/*
浅复制一个对象 也就是说只对这个对象第一层进行遍历，把这个属性拿过来复制上去
...
```javascript
对象每个属性都有一个描述对象（Descriptor），用来控制该属性的行为，Object.getOwnPropertyDescriptor()方法可以获取该属性的描述对象
可枚举性
对象字段的enumerable属性，称为可枚举性，如果该属性为false，就表示某些操作会忽略当前属性
1. forin循环：之遍历对象自身的和继承的可枚举的属性
2. Object.keys():返回对象自身的所有可枚举的属性的键名
3. JSON.stringify()只串行话对象自身的可枚举的属性
4. Object.assign()忽略不可枚举的属性，只拷贝对象自身的可枚举的属性
...
## 13. Object.is()
```javascript
我们原来已经有==和=== 为什么还要再多一种呢？
1和[1]竟然是相等的 1和"1"竟然也相等？
如果没有信心记住这些相等的值，那就永远不要用==
is是一种新的相等算法的实现
==和===的缺点
1. ==会自动转换类歘能
2. ===不认为NaN和NaN相等，认为+0和-0是相等的
is实现了"Same-value equality"（同值相等）
