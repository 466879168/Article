...
## 18. URL Params 为对象
```javascript
//又一个url地址'http://www.zhufengpeixun.cn/stu/?lx=1&name=AA&sex=man#teacher'
//先找到问号截取
//验证是否存在#哈希值
//以&符号拆分
...
## 29. 数组去重
```javascript
var arr = [1, 2, 3, 4, 5, 1, 2, 5, 6, 8, 7, 89, 45, 1, 131, 4];
//利用双循环比较
//思路：拿出第一项和后面每一项比较。一样的删除(排除最后一项)
//如果发现重复的找到重复项在原有数组中删除
//i<arr.length-1 不用拿最后一项
for (var i = 0; i < arr.length - 1; i++) {
  var item = arr[i]; //一次拿出每一项的内容
  //i 是当前拿出项的索引
  for (var k = i + 1; k < arr.length; k++) {
    //arr[k] 后面那出来和当前项比较的值
    if (item === arr[k]) {
      //相等证明重复了，拿出来k的这一项在原有数组中删除
      arr.splice(k, 1);
      k--;
    }
  }
...
//利用对象键值对去重
var arr = [1, 2, 3, 4, 5, 1, 2, 5, 6, 8, 7, 89, 45, 1, 131, 4];
//基于对象键（属性名）不能重复，实现高性能的数组去重
//1. 创建一个空对象
//2. 依次遍历数组中的每一项，把每一项存储的值当做对象的属性名和属性值存储进来，
var obj={}
for(var i=0;i<arr.length;i++){
var item=arr[i]
//如果对象中存在这个属性了，说明当前item在对象中已经存在了，也就是当前项重复了
...
## 30. 时间字符串格式化
```javascript
//又一个时间字符串'2018-4-4 16:26:8'基于这个字符串获取到'04月04日 16时26分'
var str='2018-4-4 16:26:8'
//1. 基于split按照空格把字符串拆分成两部分，左边这一部分按杠拆除 右边的按照冒号来拆
//2. 拼接
...
var month=addZero(arrLeft[1])//月
var day=addZero(arrLeft[2])//日
var hour=addZero(arrRight[0])//小时
var minute=addZero(arrRight[1])//分
